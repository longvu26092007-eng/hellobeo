...
getgenv().Settings = getgenv().Settings or {
    ...
    ["Races"] = { -- bật race nào thì script sẽ check V3 race đó, đủ hết sẽ đổi folder
        ["Human"] = true;
        ["Mink"] = true;      
        ["Fishman"] = true;   
        ["Skypiea"] = true;   
...
-- ==========================================
-- [ RACE V3 TITLE MULTI-CHECK - CACHED 30s ]
-- Chỉ CONFIRMED mới được tính là đã V3.
-- FOUND_ONLY / PROBABLE / UNKNOWN không được dùng để bỏ qua tộc.
-- Quét: getTitles + Title Number + Title Name + Obtainment
--      + Player.Titles + trạng thái/attribute/lock trong GUI Titles.
-- Không reroll và không tiêu Fragment để kiểm tra.
-- ==========================================
local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")
local RACE_TITLE_SCAN_INTERVAL = 30

local TARGETS = {
    {number = 8,  numberText = "#008", race = "Human",  raceV3 = "Human V3",  version = 3, title = "Full Power",         obtainment = "Unlock Human V3."},
    {number = 9,  numberText = "#009", race = "Rabbit", raceV3 = "Rabbit V3", version = 3, title = "Godspeed",           obtainment = "Unlock Rabbit V3."},
    {number = 10, numberText = "#010", race = "Shark",  raceV3 = "Shark V3",  version = 3, title = "Warrior of the Sea", obtainment = "Unlock Shark V3."},
    {number = 11, numberText = "#011", race = "Angel",  raceV3 = "Angel V3",  version = 3, title = "Perfect Being",      obtainment = "Unlock Angel V3."},
    {number = 12, numberText = "#012", race = "Ghoul",  raceV3 = "Ghoul V3",  version = 3, title = "Hell Hound",          obtainment = "Unlock Ghoul V3."},
    {number = 13, numberText = "#013", race = "Cyborg", raceV3 = "Cyborg V3", version = 3, title = "War Machine",        obtainment = "Unlock Cyborg V3."},
    {number = 15, numberText = "#015", race = "Draco",  raceV3 = "Draco V3",  version = 3, title = "Ancient Flame",      obtainment = "Unlock Draco V3."},
}

local TRUE_FIELDS = {
    unlocked = true, isunlocked = true,
    owned = true, isowned = true,
    obtained = true, isobtained = true,
    acquired = true, isacquired = true,
    hastitle = true, claimed = true, isclaimed = true,
}

local LOCK_FIELDS = {locked = true, islocked = true}
local ID_FIELDS = {id = true, index = true, number = true, titlenumber = true, titleid = true}
...
local LOCKED_WORDS = {
    ["locked"] = true, ["not owned"] = true,
    ["not unlocked"] = true, ["???"] = true,
}
...
local function RunRaceTitleV3Scan(force)
    if RaceTitleV3Cache.scanning then
        ...
        until not RaceTitleV3Cache.scanning or tick() >= timeoutAt
        return RaceTitleV3Cache.map
    end

    if not force
        and RaceTitleV3Cache.initialized
        and tick() - RaceTitleV3Cache.lastScan < RACE_TITLE_SCAN_INTERVAL
    then
        return RaceTitleV3Cache.map
    end

    RaceTitleV3Cache.scanning = true

    local okScan, scanError = xpcall(function()
        local results = {}
        for _, target in ipairs(TARGETS) do
            table.insert(results, newResult(target))
        end

        local remoteOk, remoteData = invokeGetTitles(1.5)
        ...
        pcall(function()
            inspectGuiFast(results)
        end)

        if titlesFrame and previousVisible ~= nil then
            pcall(function()
        ...
        RaceTitleV3Cache.status = newStatus
        RaceTitleV3Cache.evidence = newEvidence
    ...
    RaceTitleV3Cache.scanning = false

    if not okScan then
        warn("[RaceTitleV3Scan] " .. tostring(scanError))
    end
...
local function GetRaceTitleV3MapCachedInternal()
    ...
    if tick() - RaceTitleV3Cache.lastScan >= RACE_TITLE_SCAN_INTERVAL
        and not RaceTitleV3Cache.scanning
    then
        task.spawn(function()
            RunRaceTitleV3Scan(true)
        end)
    end
...
return GetRaceTitleV3MapCachedInternal
-- ==========================================
...
-- Title V3 được quét đa nguồn và cache mỗi 30 giây.
